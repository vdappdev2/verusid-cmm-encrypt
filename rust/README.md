# verusid-cmm-encrypt — Rust core

Pure-Rust writer library for Verus identity **content-multimap** entries in the
`flags:13` public-decrypt envelope shape. This is the Rust crate that powers
the TypeScript / JavaScript package at the repo root (see the
[root README](../README.md) for the TS API).
...
WebAssembly bindings live in [`../wasm/`](../wasm) and are the layer the
TypeScript wrapper consumes.
...
Both commands assume `cwd` is the `rust/` directory. Then feed each printed
block to a running daemon:
