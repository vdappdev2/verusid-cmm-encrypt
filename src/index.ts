...
function normalizePlaintext(
  ...
  if (typeof p !== 'object') {
    throw new Error(
      `plaintext must be a string, Buffer/Uint8Array, or object; got ${typeof p}`,
    );
  }
  ...
  if (ArrayBuffer.isView(p) || p instanceof ArrayBuffer) {
    throw new Error(
      `plaintext binary input must be a Buffer or Uint8Array; got ${(p as object).constructor.name}`,
    );
  }
  ...
  if (!isPlainObject && !Array.isArray(p)) {
    throw new Error(
      `plaintext object must be a plain object or array; got ${(p as object).constructor?.name ?? 'unknown'}`,
    );
  }
  ...
  if (
    Object.hasOwn(p, 'toJSON') &&
    typeof (p as { toJSON?: unknown }).toJSON === 'function'
  ) {
    throw new Error(
      'plaintext object must not define a toJSON() method — it would silently transform the payload before encryption',
    );
  }
